...
import { stake, unstake } from "../../../../lib/web3/builder";
...
import { getCollectivesThunk } from "./getCollectivesThunk";
import {
...
export const stakeThunk: AppActionThunk =
  (amount: BigNumber, isStake: boolean) => async (dispatch, getState) => {
    try {
      ...
      const decimals = await fetchTokenDecimals(proxy!)

      method = isStake
        ? stake(amount.shiftedBy(decimals.toNumber()), proxy!)
        : unstake(amount.shiftedBy(decimals.toNumber()), proxy!);
          ...
          .once("sending", () => {
            dispatch(
              setTransactionStatus({
                ...
                type: isStake ? TransactionTypes.stake : TransactionTypes.unstake,
              })
            );
          })
          .once("transactionHash", (hash: string) => {
            dispatch(
              setTransactionStatus({
                ...
                type: isStake
                  ? TransactionTypes.stake
                  : TransactionTypes.unstake,
              })
            );
          })
      ...
      console.warn("stakeThunk.error", error);
    }
  };
